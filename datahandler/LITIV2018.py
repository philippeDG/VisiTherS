...
class LITIV2018(Dataset):
    ...
    def _prepare(self) -> (DefaultDict[str, List[str]], DefaultDict[str, List[str]], DefaultDict[str, List[str]],
        ...
        for video in videos:
            ...
            rectified_root = os.path.join(im_root, 'stcharles2018-v04', 'rectified', video)
            ...
            if MASK_RCNN:
                mask_output = os.path.join(litiv_output, video)
                rgb_masks = os.path.join(mask_output, 'rgb')
                lwir_masks = os.path.join(mask_output, 'lwir')
            else:
